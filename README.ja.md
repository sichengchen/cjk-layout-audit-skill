...
この repository には `cjk-layout-audit` skill source が含まれます。Codex plugin は独立した marketplace repository からインストールしてください。

```bash
codex plugin marketplace add sichengchen/codex-plugins
```

Marketplace repository は、この skill を `cjk-text-layout` Codex plugin として package します。
