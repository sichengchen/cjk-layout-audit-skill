...
이 repository에는 `cjk-layout-audit` skill source가 들어 있습니다. Codex plugin은 별도의 marketplace repository에서 설치하세요.

```bash
codex plugin marketplace add sichengchen/codex-plugins
```

Marketplace repository는 이 skill을 `cjk-text-layout` Codex plugin으로 package합니다.
